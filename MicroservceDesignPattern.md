...
| **Circuit Breaker** | ⭐⭐⭐⭐⭐ | Stops calling a failing service after repeated failures and returns a fallback response. | Payment Service is down; the Circuit Breaker opens, so Order Service immediately returns "Payment unavailable" instead of waiting for timeouts. | Fault tolerance |
| **Retry + Timeout** | ⭐⭐⭐⭐⭐ | Retries temporary failures and stops waiting after a defined timeout. | Payment API times out after 3 seconds; retry up to 3 times with exponential backoff before failing. | Handling transient failures |
...
| **CQRS (Command Query Responsibility Segregation)** | ⭐⭐⭐⭐ | Separates write operations (Commands) from read operations (Queries), often using different models or databases. | Orders are written to PostgreSQL, but customer dashboards read from Elasticsearch for fast searching. | Read/write optimization |
| **Event Sourcing** | ⭐⭐⭐ | Stores every state change as an immutable event instead of storing only the latest state. | Instead of `Status=Delivered`, store `OrderCreated → PaymentCompleted → Packed → Shipped → Delivered`. | Audit trails |
| **Outbox Pattern** | ⭐⭐⭐⭐⭐ | Stores events in an Outbox table within the same database transaction, then publishes them asynchronously to a message broker. | Save Order and an Outbox record in one transaction; a background process later publishes `OrderCreated` to Kafka. | Reliable event publishing |
| **Event-Driven Architecture (EDA)** | ⭐⭐⭐⭐⭐ | Services communicate by publishing and consuming events instead of making direct synchronous API calls. | Order Service publishes `OrderCreated`; Inventory, Shipping, Notification, and Analytics services consume the event independently. | Asynchronous communication |
| **Strangler Fig Pattern** | ⭐⭐⭐⭐ | Gradually replaces parts of a monolithic application with microservices instead of rewriting everything at once. | Move the Product module from the monolith to a Product Service while other modules remain in the monolith. | Monolith migration |
| **Sidecar Pattern** | ⭐⭐⭐ | A helper container deployed alongside the application container to provide common infrastructure capabilities. | An Envoy sidecar handles mTLS, logging, and traffic management while the application focuses only on business logic. | Kubernetes/service mesh |
